...
function __construct(){
	...
	function form_desa($id=''){
		...
		if($id){
			$temp = $this->wilayah_model->cluster_by_id($id);
			$data['desa']	  = $temp['desa'];
			$data['individu'] = $this->wilayah_model->get_penduduk($temp['id_kades']);
			if(empty($data['individu']))
				$data['individu'] = NULL;
			else{
				$ex = $data['individu'];
			...
			$data['form_action'] = site_url("sid_core/update_desa/$id");
		}
		else{
			$data['desa']          = null;
			$data['form_action'] = site_url("sid_core/insert_desa");
		}
		$nav['act']= 5;
		$header = $this->header_model->get_data();
		$header['modul_ini'] = $this->modul_ini;
		$this->load->view('header',$header);
		$this->load->view('sid/nav',$nav);
		$this->load->view('sid/wilayah/desa_form',$data);
		$this->load->view('footer');
	}
