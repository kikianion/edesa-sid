...
class Hom_Desa extends CI_Controller{
	...
	function __construct(){
		...
		//abdulrohman
		$desa	= $this->user_model->sesi_desa($_SESSION['sesi']);
	...
	function index(){
		...
		if($_SESSION['grup']==2){
			$data['main'] = $this->config_model->get_data($_SESSION['desa']);
	...
	function form($ids = NULL){
		...
		if($_SESSION['grup']==2){
			...
			$data['main'] = $this->config_model->get_data($ids);
		}
	...
	function tambah_desa($ids = 0){
		...
		$this->load->view('home/nav',$nav);
		// Buat row data desa di konfigurasi_form apabila belum ada data desa
		$data['form_action'] = site_url("hom_desa/insert_desa/");
		$this->load->view('home/konfigurasi_form',$data);
		$this->load->view('footer');
	}
