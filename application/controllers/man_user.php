...
class Man_User extends CI_Controller{
	...
	//abdulrohman
	function login_as(){
		//$data['form_action'] = site_url("man_user/login_as");
		//$header = $this->header_model->get_data();
		//$data['desa'] = $this->penduduk_model->list_desa();
		$this->user_model->login_as();
	...
	//abdulrohman form login_as
	function form_login_as(){
		...
		$this->load->view('header', $header);
		//$this->load->view('man_user/nav');
		$this->load->view('man_user/form_login_as',$data);
		$this->load->view('footer');
	}
