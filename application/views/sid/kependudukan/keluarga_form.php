...
<tr>
  <th width="100"><?php echo ucwords(config_item('sebutan_desa'))?></th>
  <td><select name="desa" onchange="formAction('mainform','<?php echo site_url('keluarga/form/0/1')?>')" <?php if($desa){?>class="required"<?php }?>>
    <option value="">Pilih <?php echo ucwords(config_item('sebutan_desa'))?></option>
    <?php foreach($desa as $data){?>
      <option value="<?php echo $data['desa']?>" <?php if($des_sel==$data['desa']){?>selected<?php }?>><?php echo unpenetration(ununderscore($data['desa']))?></option>
    <?php }?></select>
  </td>
</tr>
<tr>
