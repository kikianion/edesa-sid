<?php class Pamong_Model extends CI_Model{
	...
	function list_data(){
		$sql   = "SELECT u.* FROM tweb_desa_pamong u ";

		if($_SESSION['grup']!=1){
			$sql .= " WHERE u.desa='".$_SESSION['desa']."'";
		}else{
			$sql .=" WHERE 1";
		}
	...
	function insert(){
		...
		$sql = "INSERT INTO tweb_desa_pamong (pamong_nama,pamong_nip,pamong_nik,jabatan,pamong_status,pamong_tgl_terdaftar,desa)
				VALUES (?,?,?,?,?,NOW(),?)";

		$outp = $this->db->query($sql, array($nama,$nip,$nik,$jabatan,$status,$desa));
