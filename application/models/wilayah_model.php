<?php class Wilayah_Model extends CI_Model {
	...
	function insert(){
		...
		$data['id_kepala'] = 0;
		$data = $_POST;
		if(empty($data['id_kepala'])){
			$data['id_kepala'] = 0;
		}
		$data['dusun']=underscore($_POST['dusun']);
		//abdulrohman
		$data['desa']=underscore($_POST['desa']);
		//if($_SESSION['grup'==1]){
		//	$data['desa']=underscore($_POST['desa']);
		//}
		if($_SESSION['grup'==2]){
			$data['desa']=underscore($_SESSION['desa']);
		}
	...
	function update_desa($id=''){
		if(empty($_POST['id_kades']))
			UNSET($_POST['id_kades']);
