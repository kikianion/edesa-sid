<?php class Penduduk_Model extends CI_Model{
	...
	function get_alamat_wilayah($id) {
		...
		$sql = "SELECT a.desa,a.dusun,a.rw,a.rt,u.alamat_sekarang as alamat
	...
	private function list_data_sql($log) {
		...
		if($_SESSION['grup']==2){
			$sql .=" WHERE a.desa='".$_SESSION['desa']."'";
	...
	function list_data($o=0,$offset=0,$limit=500,$log=0){
		...
		if ($log==1) {
			$select_sql = "SELECT u.id,u.nik,u.tanggallahir,u.tempatlahir,u.status,u.status_dasar,u.id_kk,u.nama,u.nama_ayah,u.nama_ibu,a.desa, a.dusun,a.rw,a.rt,d.alamat,log.no_kk AS no_kk,log.catatan as catatan,log.nama_kk as nama_kk,
				(SELECT DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(`tanggallahir`)), '%Y')+0 FROM tweb_penduduk WHERE id = u.id) AS umur,x.nama AS sex,sd.nama AS pendidikan_sedang_idang,n.nama AS pendidikan,p.nama AS pekerjaan,k.nama AS kawin,g.nama AS agama,m.nama AS gol_darah,hub.nama AS hubungan,log.tanggal,log.tgl_peristiwa,log.id_detail
				";
		} else {
			// data log tidak di-select, supaya di tabel Penduduk tidak ada duplikat
			$select_sql = "SELECT DISTINCT u.id,u.nik,u.tanggallahir,u.tempatlahir,u.status,u.status_dasar,u.id_kk,u.nama,u.nama_ayah,u.nama_ibu,a.desa,a.dusun,a.rw,a.rt,d.alamat,d.no_kk AS no_kk,
		...
		while($i<count($data)){
			...
			if (!$data[$i]['id_kk'] OR $data[$i]['id_kk'] == 0) {
				...
				$sql = "SELECT p.id_cluster, p.alamat_sekarang, c.desa,c.dusun, c.rw, c.rt
	...
	function insert(){
		...
		//abdulrohman
		$temp['desa'] = $data['desa'];
		$temp['nama'] = penetration($data['nama']);
		$temp['username'] = $data['nik'];
		UNSET($data['desa']);
		...
		$akun['id_grup'] = 5;
		$akun['desa'] = $temp['desa'];
		$akun['session'] = $temp['username'];
		$akun['email'] = ''; //default value
		$akun['username'] = $temp['username'];
		$akun['password'] = md5('123456'); //default password for user penduduk

		$sqll2 = $this->db->insert('user',$akun);
	...
	function update($id=0){
		...
		//abdulrohman 
		if(empty($data['cara_kb_id'])){
			unset($data['cara_kb_id']);
		}
		if(empty($data['dokumen_kitas'])){
			unset($data['dokumen_kitas']);
		}
		if(empty($data['golongan_darah_id'])){
			unset($data['golongan_darah_id']);
		}
		if(empty($data['cacat_id'])){
			unset($data['cacat_id']);
		}
	...
	function get_penduduk($id=0){
		$sql   = "SELECT u.sex as id_sex,u.*,a.desa,u.bayar_pbb,u.miskin,a.dusun,a.rw,a.rt,t.nama AS status,a.desa,o.nama AS pendidikan_sedang,
	...
	function list_dusun($desa=''){
		$sql   = "SELECT * FROM tweb_wil_clusterdesa WHERE rt = '0' AND desa = ? AND rw = '0' ";
/*
		if($_SESSION['grup'] !== 1){
			$sql .= " AND desa=?";
			$query = $this->db->query($sql,$_SESSION['desa']);
		}else{
			$query = $this->db->query($sql);
		}
		*/
		$query = $this->db->query($sql,$desa);
		$data=$query->result_array();
		//echo $this->db->last_query();
		return $data;
	}
	...
	//abdulrohman
	function list_desa(){
		$sql   = "SELECT DISTINCT desa FROM tweb_wil_clusterdesa WHERE rt = '0' AND rw = '0' ";
