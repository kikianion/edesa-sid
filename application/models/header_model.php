...
class Header_Model extends CI_Model{
	...
	//abdulrohman
	function get_desa($user=''){
		$sql   = "SELECT desa FROM user WHERE username=?";
		$query = $this->db->query($sql,$user);
		$data = $query->row_array();
		return $data['desa'];
	}
	 ...
	 * */
		...
		$sql   = "SELECT * FROM config WHERE desa=?";
		...
		$outp['modul'] = $modul;
		//abdul rohman khuusus super admin:
		if($_SESSION['grup'] == 1){
			$outp['desa']['nama_desa'] = 'Keseluruhan';	
			$outp['desa']['nama_kecamatan'] = 'Keseluruhan';	
			$outp['desa']['logo'] = $this->config->item('logo');
		}
