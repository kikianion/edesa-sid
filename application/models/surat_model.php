<?php class Surat_Model extends CI_Model{
	...
	function list_penduduk(){
		...
		//abdulrohman
		if($_SESSION['grup']!=1){
			$sql .= " WHERE w.desa = '".$_SESSION['desa']."'";
		}
	...
	*/
	...
	//abdulrohman impossible to parsing docx
	function surat_docx($data){
		// Ambil data
		$input = $data['input'];
		$individu = $data['individu'];
		$ayah = $data['ayah'];
...
//      $html2pdf->setModeDebug();
	...
	function get_last_nosurat_log($url){
		...
		if (config_item('nomor_terakhir_semua_surat')){
			...
			$sql   = "SELECT no_surat,tanggal FROM log_surat LEFT JOIN user ON log_surat.id_user = user.id WHERE id_format_surat = ? ";
			if($_SESSION['grup'] != 1){
				$sql .= " AND user.desa = '".$_SESSION['desa']."'";
			}
