<?php class Keluarga_Model extends CI_Model{
	...
	function list_data($o=0,$offset=0,$limit=500){
		...
		$sql   = "SELECT u.*,t.nama AS kepala_kk,t.nik,t.sex,t.status_dasar,(SELECT COUNT(id) FROM tweb_penduduk WHERE id_kk = u.id AND status_dasar = 1) AS jumlah_anggota,c.desa,c.dusun,c.rw,c.rt
		...
		if($_SESSION['grup']!=1){
			$sql .= " WHERE c.desa = '".$_SESSION['desa']."'";
	...
	function insert(){
		...
		//abdulrohman set default for id_cluster
		$sql = "SELECT p.id_cluster AS cluster FROM tweb_penduduk p WHERE p.id=?";
		$query = $this->db->query($sql,$temp);
		$idcluster = $query->row_array();

		//echo $this->db->last_query();

		$data['id_cluster'] = $idcluster['cluster'];
		//end of default id_cluster
